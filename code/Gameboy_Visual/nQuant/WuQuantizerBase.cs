...
// King Kadelfek : the script is crudely edited to:
//- allow only 255 colors
//- do not check the pixel format
//- send back only the palette instead of a new picture
namespace nQuant
{
    public abstract class WuQuantizerBase
    {
        private int MaxColor = 6;
        ...
        public void set_max_color (int max_color)
        {
            this.MaxColor = max_color + 1;
        }

        // MODIFICATION
        public QuantizedPalette QuantizeImage (byte[] arr_color, int bitmapWidth, int bitmapHeight)
        {
            return QuantizeImage(arr_color, bitmapWidth, bitmapHeight, 10, 70);
        }

        // MODIFICATION
        public QuantizedPalette QuantizeImage (byte[] arr_color, int bitmapWidth, int bitmapHeight, int alphaThreshold, int alphaFader)
        {
            int colorCount = MaxColor;
            ColorData color_data = BuildHistogram (arr_color, bitmapWidth, bitmapHeight, alphaThreshold, alphaFader);
            color_data = CalculateMoments (color_data);
            IEnumerable<Box> cubes = SplitData (ref colorCount, color_data);
            QuantizedPalette palette = GetQuantizedPalette (colorCount, color_data, cubes, alphaThreshold);
            return palette;
            //return ProcessImagePixels (arr_color, palette);
        }

        /* MODIFICATION
        private static Bitmap ProcessImagePixels (int bitmapWidth, int bitmapHeight, QuantizedPalette palette)
        {
            var result = new Bitmap (bitmapWidth, bitmapHeight, PixelFormat.Format8bppIndexed);
        ...
        */
        ...
        // MODIFICATION
        private static ColorData BuildHistogram (byte[] buffer, int bitmapWidth, int bitmapHeight,
            int alphaThreshold, int alphaFader)
        {

            ColorData colorData = new ColorData(MaxSideIndex, bitmapWidth, bitmapHeight);


            var bitDepth = 32;// Image.GetPixelFormatSize (sourceImage.PixelFormat); // MODIFICATION
            if (bitDepth != 32)
            ...
            var byteLength = bitmapWidth * 4; //data.Stride < 0 ? -data.Stride : data.Stride;
            var byteCount = Math.Max(1, bitDepth >> 3);
            var offset = 0;
            //var buffer = new Byte[byteLength * sourceImage.Height]; // MODIFICATION
